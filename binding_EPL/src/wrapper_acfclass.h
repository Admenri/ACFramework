...
class AcfRequestResponseWrapper : public AcfResourceHandler {
 public:
  AcfRequestResponseWrapper(const void* buffer, size_t size,
                            AcfRefPtr<AcfResponse> response, int64_t length,
                            std::string redirect);
 ...
 protected:
  ...
  void Read(void* data_out, int bytes_to_read,
...
class ACFResourceFilterHandler : public AcfResponseFilter {
 public:
  ACFResourceFilterHandler(LPVOID callback, AcfRefPtr<AcfProfile> profile,
                       int64_t frame_id, AcfRefPtr<AcfRequest> request,
